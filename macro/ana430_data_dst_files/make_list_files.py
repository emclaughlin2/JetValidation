...
directory = '/sphenix/user/egm2153/calib_study/JetValidation/macro/data_dst_files'
...
for filename in os.listdir(directory):
    if filename.endswith('.list'):
        ...
        # Split the lines into chunks of 10 lines
        for i in range(0, len(lines), 10):
            chunk = lines[i:i+10]
            
            # Generate the new filename with a chunk index
            new_filename = f"{filename[:-5]}-{i//10:05d}.list"
